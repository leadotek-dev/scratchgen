...
fn build_images(config: &Config, pool_patterns: &[String]) -> Result<Vec<ImgMeta>> {
    ...
    if files.is_empty() {
        anyhow::bail!("No images found in pool");
    }
    ...
    for f in files {
        ...
        if let Some(ws) = weights {
            if let Some(v) = ws.get(&f.to_string_lossy().to_string()) {
                w = *v;
            }
            if let Some(v) = ws.get(&bn) {
                w = *v;
            }
        }
        let min = mins
            .and_then(|m| m.get(&f.to_string_lossy().to_string()).cloned())
            .or_else(|| mins.and_then(|m| m.get(&bn).cloned()))
            .unwrap_or(0);
        let max = maxs
            .and_then(|m| m.get(&f.to_string_lossy().to_string()).cloned())
            .or_else(|| maxs.and_then(|m| m.get(&bn).cloned()))
            .unwrap_or(i32::MAX);
        imgs.push(ImgMeta {
            path: f,
            weight: if w > 0.0 { w } else { 0.0001 },
            min,
            max,
        });
    }
...
fn sample_without_replacement(
    ...
    if k > n && !allow_repeat {
        panic!(
            "Pool has fewer than {} images and repeats are disallowed",
            k
        );
    }
...
fn allocate_constrained(
    ...
    for _iter in 0..100 {
        ...
        for i in 0..n {
            ...
            if alloc[i] < min {
                ...
                if need > 0 {
                    panic!("Constraints infeasible: cannot satisfy min_count requirements");
                }
            }
        }
        // maxs
        for i in 0..n {
            ...
            if alloc[i] > max {
                ...
                if excess > 0 {
                    panic!("Constraints infeasible: cannot redistribute excess from max_count");
                }
            }
        }
        if !changed {
    ...
    if alloc.iter().sum::<i32>() != total_tiles as i32 {
        panic!("Failed to allocate counts summing to {}", total_tiles);
    }
    let mut selected: Vec<PathBuf> = Vec::new();
    for i in 0..n {
        for _c in 0..alloc[i] {
            selected.push(images[i].path.clone());
        }
    }
...
fn sample_independent(images: &[ImgMeta], k: usize, rng: &mut ChaCha8Rng) -> Vec<PathBuf> {
    let mut res = Vec::new();
    for _ in 0..k {
        let idx = weighted_pick(images, rng);
        res.push(images[idx].path.clone());
    }
    res
}
...
fn compose_grid(
    ...
    // If a background image is provided, use it (resized to cover). Otherwise create a solid-color canvas.
    let mut canvas: RgbaImage;
    if let Some(bg_path) = background_image {
        let img = image::open(bg_path)
            .with_context(|| format!("Failed to open background image {:?}", bg_path))?;
        let resized = img.resize_to_fill(w, h, FilterType::Lanczos3);
        canvas = resized.to_rgba8();
    } else {
        canvas = RgbaImage::from_pixel(w, h, bg);
    }
    for i in 0..total {
        let p = &paths[i];
...
fn main() -> Result<()> {
    ...
    // merge CLI overrides
    let pool_patterns: Vec<String> = if !cli.pool.is_empty() {
        cli.pool.clone()
    } else {
        config.pool.clone().unwrap_or_default()
    };
    let mode = if cli.mode != "constrained" {
        cli.mode.clone()
    } else {
        config.mode.clone().unwrap_or(cli.mode.clone())
    };
    let tile_size = if cli.tile_size != 256 {
        cli.tile_size
    } else {
        config.tile_size.unwrap_or(cli.tile_size)
    };
    let fit = if cli.fit != "cover" {
        cli.fit.clone()
    } else {
        config.fit.clone().unwrap_or(cli.fit.clone())
    };
    let seed = if cli.seed != 0 {
        cli.seed
    } else {
        config.seed.unwrap_or(cli.seed)
    };
    let output = if let Some(o) = cli.output {
        o
    } else if let Some(o2) = config.output.clone() {
        PathBuf::from(o2)
    } else {
        anyhow::bail!("--output required")
    };
    let background = if cli.background != "#000000" {
        cli.background.clone()
    } else {
    ...
    let allow_repeat = if cli.allow_repeat_when_pool_small {
        ...
        config.allow_repeat_when_pool_small.unwrap_or(false)
    };
    ...
    let selected: Vec<PathBuf> = match mode.as_str() {
        ...
        "without_replacement" => {
            sample_without_replacement(&images, tiles_count, &mut rng, allow_repeat)
        }
    ...
    let bg_image_path: Option<PathBuf> = if let Some(bi) = cli.background_image.clone() {
        Some(bi)
    } else if let Some(bi2) = config.background_image.clone() {
        Some(PathBuf::from(bi2))
    } else {
        None
    };
    compose_grid(
